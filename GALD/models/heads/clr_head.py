...
from clrnet.models.losses.curve_loss import curve_loss
from clrnet.models.losses.angle_loss import angle_loss
...
class CLRHead(nn.Module):
    ...
    def loss(self,
        ...
        if self.cfg.haskey('curve_loss_weight'):
            curve_loss_weight = self.cfg.curve_loss_weight
        if self.cfg.haskey('upper_loss_weight'):
            upper_loss_weight = self.cfg.upper_loss_weight
        if self.cfg.haskey('low_loss_weight'):
            low_loss_weight = self.cfg.low_loss_weight
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                ys = (self.prior_ys * (self.cfg.ori_img_h - self.cfg.cut_height) +
                       self.cfg.cut_height) / self.cfg.ori_img_h
                c_loss, u_loss = curve_loss(ys, reg_pred, reg_targets, self.img_w)
                k_loss = k_loss + c_loss

                upper_loss = upper_loss + u_loss
        ...
        k_loss /= (len(targets) * self.refine_layers)
        upper_loss /= (len(targets) * self.refine_layers)

        loss = cls_loss * cls_loss_weight + reg_xytl_loss * xyt_loss_weight \
            + seg_loss * seg_loss_weight + iou_loss * iou_loss_weight + k_loss * curve_loss_weight + upper_loss * upper_loss_weight
        ...
        return_value = {
            ...
            'loss_stats': {
                ...
                'k_loss': k_loss * curve_loss_weight,
                'upper_loss': upper_loss * upper_loss_weight,
