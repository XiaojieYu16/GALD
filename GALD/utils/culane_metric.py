...
def eval_predictions(pred_dir,
    ...
    ret = {}
    fp_paths = {thr: [] for thr in iou_thresholds}
    fn_paths = {thr: [] for thr in iou_thresholds}

    with open(list_path, 'r') as file_list:
        filepaths = [
            os.path.join(
                anno_dir, line[1 if line[0] == '/' else 0:].rstrip().replace(
                    '.jpg', '.lines.txt')) for line in file_list.readlines()
        ]
    ...
    for thr in iou_thresholds:
        ...
        fp = 0
        fn = 0
        for idx, (m, pred, anno) in enumerate(zip(results, predictions, annotations)):
            tp += m[thr][0]
            fp += m[thr][1]
            fn += m[thr][2]
            if m[thr][1] > 0:
                fp_paths[thr].append(filepaths[idx])
            if m[thr][2] > 0:
                fn_paths[thr].append(filepaths[idx])
        ...
        # Save fn_paths and fp_paths to txt files
        # with open(f'fn_paths_{thr}.txt', 'w') as fn_file:
        #     for path in fn_paths[thr]:
        #         fn_file.write(path + '\n')
        
        # with open(f'fp_paths_{thr}.txt', 'w') as fp_file:
        #     for path in fp_paths[thr]:
        #         fp_file.write(path + '\n')
